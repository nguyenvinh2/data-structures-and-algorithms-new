...
let biggerThanLuke = (arr) => {
  // Solution code here...
  return arr.filter(object => parseInt(object.mass) > parseInt(arr[0].mass)).map(object => object.name).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  win.push([board[0][0], board[1][0], board[2][0]]);
  win.push([board[0][1], board[1][1], board[2][1]]);
  win.push([board[0][2], board[1][2], board[2][2]]);
  win.push([board[0][0], board[1][1], board[2][2]]);
  win.push([board[2][0], board[1][1], board[0][2]]);

  for (let i = 0; i < win.length; i++) {
    if (win[i].every(tic => tic === 'X') === true) {
      return true;
    } else if (win[i].every(tic => tic === 'O') === true) {
      return true;
    }
  }
  return false;
