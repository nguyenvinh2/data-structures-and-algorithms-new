...
const returnNames = (arr) => {
  // Solution code here...
  return arr.reduce((arr, family) => arr.concat(family.name), []);
};
...
const reversedString = (arr) => {
  // Solution code here...
  return arr.split('').reduce((word, current, index, arr) => word.concat(arr[arr.length-index-1]), []).join('');
};
