...
const isCapitalized = (str) => {
  // Solution code here...
  return str.split(' ').map(element => element.replace(/[,()'";:!@#$%^&*.<>?]/g, '')).filter(element => element.match(/^[A-Z]/));
};
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  return (arr.filter(element => element.match(/^[A-J]/)));
};
...
const noPunctuation = str => {
  // Solution code here...
  return str.toString().match( /(\w+\s)/g);
};
