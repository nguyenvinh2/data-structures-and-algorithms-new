...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((sum, curr) => sum + curr.reduce((sum, curr) => curr === target? sum + 1: sum, 0), 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((sum, curr) => sum + curr.reduce((sum, curr) => sum + curr, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr => arr.filter(ele => !(ele%5) && typeof ele === 'number').map(ele => Math.pow(2, ele)));
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(character => ['male', 'female'].includes(character.gender)).map(character => character.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  return data.reduce((sum, curr) => parseInt(curr.height) < parseInt(sum.height)? curr: sum).name;
};
